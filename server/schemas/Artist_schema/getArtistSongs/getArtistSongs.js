  ...
  try {
    ...
    if (client) {
      ...
      console.log('📊 Redis cache result:', { 
        songIds, 
        songIdsLength: songIds?.length || 0,
        songIdsSample: songIds?.slice(0, 3) || []
      });
      
      if (songIds && songIds.length > 0) {
        console.log(`✅ Cache hit for artist ${artistId}: ${songIds.length} songs`);
        
        console.log('📋 Building MongoDB query for cached song IDs...');
        const mongoQuery = {
          _id: { $in: songIds },
          visibility: 'public',
          artist: artistId
        ...
        console.log(`📦 Retrieved ${songs.length} songs from DB for cache`);
    ...
    // 2. If Redis cache miss or empty, fallback to database
    if (songs.length === 0) {
      console.log(`🔄 Cache miss/empty, querying database directly for artist ${artistId}...`);
